...
void main() {
  ...
  testWidgets('в строке списка заголовок — поставщик, под ним сумма', (tester) async {
    final api = _FakeApi()
      ..results = [
        {
          'id': 1,
    ...
    expect(money.dy, greaterThan(supplier.dy), reason: 'сумма должна быть под заголовком');
    expect(money.dx, supplier.dx, reason: 'сумма должна стоять под заголовком, а не справа');
  });
  ...
  testWidgets('накладные разделены по дням, в строках только время', (tester) async {
    final api = _FakeApi()
      ..results = [
  ...
  testWidgets('длинное название поставщика не рвёт вёрстку', (tester) async {
    ...
    final api = _FakeApi()
      ..results = [
        row(
          id: 1,
          supplier: 'Товарищество с ограниченной ответственностью "KARAVAN (КАРАВАН)"',
