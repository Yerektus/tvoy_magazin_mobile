...
class DocumentsStore extends ChangeNotifier {
  ...
  ///
  /// Строка заводится пустой и встаёт первой — так её видно сразу, без
  /// прокрутки длинной таблицы. Заполняют её уже в карточке позиции.
  ///
  /// Возвращает накладную целиком: сервер перенумеровал остальные строки.
  Future<DocumentDetail> addLine(int invoiceId) async {
    await _api.post('/invoices/$invoiceId/lines/', const {});
  ...
  /// Выкинуть накладную.
  ///
  /// Снятую не тот стороной, чужую, задвоенную — такие в списке только мешают.
  /// На сервере она не стирается, а уходит из выдачи, поэтому список после
  /// удаления перечитываем: там она уже не появится.
